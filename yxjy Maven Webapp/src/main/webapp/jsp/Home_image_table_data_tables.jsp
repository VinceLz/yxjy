			...
			<div class="col-sm-12">
				<div class="ibox float-e-margins">
					...
					<div class="ibox-content">
						<div class="">
							<a onclick=""
								href="<%=application.getContextPath()%>/jsp/Image_form_basic.jsp"
						...
						<table
							...
							<tbody>
								<c:forEach items="${list}" var="item">
									<tr class="gradeX">
										<td><c:if test="${item.type eq 1 }">小图</c:if><c:if
												test="${item.type eq 2 }">大图</c:if>
										</td>
										<td><img width="100" height="100"
											src="<%=application.getContextPath()%>${item.content}" /></td>
										<td><a
											href="<%=application.getContextPath()%>/home/imgDele.action?hid=${item.hid}"
											class="btn btn-danger btn-sm pull-right deleteA">删除</a>
										</td>
									</tr>
								</c:forEach>
